    ...
    <div className="space-y-6 animate-pop">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-[#111827] to-slate-900 border border-white/[0.08]">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-semibold mb-2">
            ...
            <span>Klub Sepak Bola Indonesia</span>
          </div>
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-xl">
            Profil resmi, nama pelatih kepala, stadion kandang,
            jumlah skuad, dan performa rata-rata xG klub yang
            tersedia di dalam data aplikasi.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
        {LIGA1_TEAMS.map((team, idx) => {
          /*
           * TeamBadge sudah menangani normalisasi nama klub
           * untuk mengambil logo dari TEAM_INFO.
           */
          const info =
            TEAM_INFO[team.name] || {};

          /*
           * Statistik dibuat 0 terlebih dahulu.
           * Tidak menggunakan nilai dummy lama.
           */
          const played = 0;
          const points = 0;
          const avgXg = 0;
          ...
          return (
            <div
              key={team.id}
              className="p-5 rounded-3xl bg-[#111827] border border-white/[0.08] hover:border-orange-500/40 hover:-translate-y-1 transition-all duration-300 space-y-4 shadow-xl flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-black text-white text-sm group-hover:text-orange-400 transition-colors">
                        {team.name}
                      </h3>
